...
import { initL1Version, initL2Version } from "./entity-mocking/MockVersion";
import { init as bs3Init, preUnpause as bs3PreUnpause } from "../src/utils/init/B3Init";
import {
  AQUIFER,
  BEAN_ERC20,
  BEANSTALK,
...
import { init as pintoInit } from "../src/utils/init/PintoInit";
import { FieldHourlySnapshot, SiloHourlySnapshot } from "../generated/schema";
...
describe("Pinto Init", () => {
  ...
  // Intentionally using L2/Beanstalk constants for this test, for simplicity
  test("Initializes hourly snapshots", () => {
    const siloSnapshot = SiloHourlySnapshot.load(BEANSTALK.toHexString() + "-1");
    assert.assertNotNull(siloSnapshot);
    const fieldSnapshot = FieldHourlySnapshot.load(BEANSTALK.toHexString() + "-1");
    assert.assertNotNull(fieldSnapshot);
  });
});
